    ...
    const pages = [
        {
            ...
            1. Desarrollo de un ERP Web basado en Intelisis, automatizando flujos contables/operativos.
            2. Sistema de puntos con SQL Server, optimizando gestión de recompensas.
            3. Plataforma de cotización y simulación para SkyCapital, integrando requerimientos comerciales.`
        },
        {
            title: "Tecnologías",
            type: "tech-grid",
        ...
        {
            title: "Objetivos",
            content: `Como desarrollador web apasionado por la tecnología, busco integrarme en equipos dinámicos donde pueda contribuir con mis habilidades mientras aprendo de otros profesionales. Mi objetivo es enfrentar nuevos desafíos que me permitan crecer tanto técnicamente como en experiencia colaborativa, siempre con mente abierta y disposición para adaptarme a diferentes entornos.
                    
            Estoy comprometido con el aprendizaje continuo, no solo para mejorar mis capacidades en lenguajes, frameworks y herramientas actuales, sino también para explorar tecnologías emergentes que impulsen la innovación. Quiero dominar mejores prácticas de desarrollo, arquitecturas escalables y metodologías ágiles, aplicándolas en proyectos reales que generen impacto.
            
            Más allá del crecimiento individual, aspiro a también compartir conocimiento: documentar soluciones y mentorar a otros desarrolladores. Creo que la mejora constante solo cobra sentido cuando se usa para ayudar a otros y construir soluciones que simplifiquen la vida de las personas.
            
            Busco un entorno donde el intercambio de ideas, la curiosidad y la mejora continua sean valores fundamentales. Mi meta es no solo ser un mejor profesional, sino también un aliado para mi equipo y la industria.`
        }
    ]
                ...
                <div class="relative">
                    <div class="book bg-slate-800/70 rounded-xl shadow-2xl border border-slate-700/50 w-full h-[32rem] relative overflow-hidden">
                        <div class="absolute top-4 right-4 z-10">
                            <img src="/images/perfil.webp" alt="Edgar Pech" class="profile-img rounded-full w-14 h-14 md:w-16 md:h-16 object-cover shadow-lg border-3 border-indigo-500/50">
                        </div>
                        
                        <!-- Contenido principal -->
                        <div class="book-content text-gray-300 h-full flex flex-col p-6">
                            <h3 class="text-xl font-semibold mb-12 pl-3 text-indigo-400">{{ pages[currentPage].title }}</h3>
                            
                            <div v-if="pages[currentPage].type !== 'tech-grid'" class="flex-grow overflow-y-auto pr-2 scrollbar-custom">
                                <p class="text-base md:text-lg pl-3 leading-relaxed whitespace-pre-line">{{ pages[currentPage].content }}</p>
                            </div>
                            ...
                            <div v-else class="flex-grow overflow-y-auto pr-2 scrollbar-custom">
                                <div v-for="(section, sectionIndex) in pages[currentPage].sections" :key="sectionIndex" class="mb-6">
                                    <h4 class="text-sm font-medium mb-3 text-slate-400 pl-3">{{ section.title }}</h4>
                                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 pl-3">
                                        <div class="relative" v-for="(item, index) in section.items" :key="index"
                                            @mouseenter="showTooltip($event, item, section.title, index)"
                                            @mouseleave="hideTooltip"
                                            @click="showTooltip($event, item, section.title, index)"
                                        >
                                            <div class="tech-logo p-2 rounded-lg flex flex-col items-center justify-center h-full transition-all hover:scale-105"
                                                :class="item.color"
                                            >
                            ...
                            <div class="flex justify-between items-center mt-auto pt-4 border-t border-slate-700/50">
                                <div class="flex-1 min-w-0">
                                    <button class="arrow-btn w-full px-2 py-2 rounded-md transition-all text-sm md:text-base whitespace-nowrap overflow-hidden text-ellipsis"
                                        @click="prevPage"
                        ...
                        <!-- Tooltip -->
                        <div class="fixed md:absolute z-20 bg-slate-900 border border-slate-700 rounded-lg shadow-lg p-3 max-w-xs transition-opacity duration-200 pointer-events-none"
                        v-if="activeTooltip"
                        :style="{
